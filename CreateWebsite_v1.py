...
def get_stats_data(db_path):
    ...
    stats['group_dms'] = [
        {
            'recipient_count': len(json.loads(rec)),
            'count': count,
            'attachments': att
        }
        for rec, count, att in cursor.fetchall()
...
<head>
    ...
    <title>Discord Stats 2025</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
"""
    ...
    for dm in stats['dms'][:15]:
        ...
        html += f"""
                <div class="dm-item">
                    <div class="dm-id">User ID: {dm['user_id']}</div>
                    <div class="dm-stats">
...
"""
        for idx, gdm in enumerate(stats['group_dms'][:10], 1):
            percentage = (gdm['count'] / max(stats['total_messages'], 1) * 100)
            html += f"""
                <div class="bar-item">
                    <div class="bar-header">
                        <span class="bar-name">Group Chat #{idx} ({gdm['recipient_count']} members)</span>
...
"""
    ...
    html += f"""
        ...
        <footer>
            ...
            <p>Discord GDPR Data Analysis | StoiaCode</p>
        </footer>
    </div>
</body>
</html>
...
if __name__ == "__main__":
    ...
    if args.serve:
        ...
        print(f"   python {os.path.basename(__file__)} --serve")
